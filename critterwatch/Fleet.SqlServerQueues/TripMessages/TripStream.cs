...
/// trip; the publisher dequeues one command at a time and dispatches it over the SQL Server queue
/// transport. TripService handles each command, publishes <see cref="ContinueTrip"/> back, and the
/// publisher advances to the next message — a self-driving loop that keeps steady-state traffic flowing.
/// </summary>
