...
namespace Fleet.GooglePubSub.Tests;
...
{
    public const string Name = "Fleet.GooglePubSub AppHost";
}

/// <summary>
/// End-to-end smoke battery: boots the whole Fleet.GooglePubSub AppHost (the Pub/Sub emulator container +
/// Postgres + the console + the Trip trio) once, then asserts the console is up and the fleet self-registers
/// over the Pub/Sub control channel.
/// </summary>
...
public class FleetSmokeTests
{
    ...
    public async Task fleet_services_register_with_the_console()
    {
        ...
        // Services self-register ASYNCHRONOUSLY over the Pub/Sub control channel after the console is up, so
        // we poll /services rather than asserting immediately. The names are each service's ServiceName.
        // Route through the fixture so a timeout dumps every resource's captured logs into the failure
        // message — the emulator container's first-run image pull + topic/subscription provisioning are slow,
