...
public static class LogIncidentHandler
{
    ...
    public static (IStartStream, OutgoingMessages) Handle(LogIncident command, ILogger logger)
    {
        ...
        var startStream = MartenOps.StartStream<Incident>(
            command.IncidentId,
...
/// <c>[AggregateHandler]</c> wires each to <c>FetchForWriting&lt;Incident&gt;</c>, applies the event(s),
/// and forwards the <see cref="OutgoingMessages"/> through the outbox in the same transaction.
/// </summary>
[AggregateHandler]
