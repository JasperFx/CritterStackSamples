...
public record CompleteAppointment(Guid AppointmentId, string Notes);
...
public static class CompleteAppointmentEndpoint
{
    ...
    public static (CompleteAppointmentResponse, EventsToAppend) Post(CompleteAppointment command, [WriteModel] Appointment appointment)
    {
        ...
        var completed = new AppointmentCompleted(command.AppointmentId, appointment.OwnerId, command.Notes, DateTimeOffset.UtcNow);
        return (new CompleteAppointmentResponse(command.AppointmentId), [completed]);
    }
}
